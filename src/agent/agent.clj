...
(defmethod webhook :grpc [task]
  ...
  (try
    @(grpc-client/Webhook (clients/get-grpc-client) (dissoc task :mode))
...
(defn succeed-task-with-message [task message ]
  (webhook {:id (:id task) :status "success" :logs message :org clients/org :mode (:mode task)}))

(defn fail-task-with-message [task message]
  (webhook {:id (:id task) :status "failure" :logs message :org clients/org :mode (:mode task)}))
...
(defn run-task [task]
  (log/info (format "Start running task with id: %s" (:id task)))
  (err/err->> task
              parse-command
              lock-task
              get-secrets
