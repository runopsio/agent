...
; SubscriberRequest
;-----------------------------------------------------------------------------
(defrecord SubscriberRequest-record [name]
  pb/Writer
  (serialize [this os]
...
(s/def :io.grpc.SubscriberRequest/name string?)
(s/def ::SubscriberRequest-spec (s/keys :opt-un [:io.grpc.SubscriberRequest/name ]))
(def SubscriberRequest-defaults {:name "" })
...
(s/def :io.grpc.LogsRequest/org string?)
(s/def ::LogsRequest-spec (s/keys :opt-un [:io.grpc.LogsRequest/id :io.grpc.LogsRequest/status :io.grpc.LogsRequest/logs :io.grpc.LogsRequest/org ]))
(def LogsRequest-defaults {:id 0 :status "" :logs "" :org "" })
  ...
  (->> (tag-map LogsRequest-defaults
         (fn [tag index]
             (case index
               ...
               4 [:org (serdes.core/cis->String is)]
