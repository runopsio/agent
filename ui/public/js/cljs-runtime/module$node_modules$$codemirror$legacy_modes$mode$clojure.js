...
global={startState:function(){return{ctx:{prev:null,start:0,indentTo:0},lastToken:null,tokenize:base}},token:function(stream,state){stream.sol()&&"number"!==typeof state.ctx.indentTo&&(state.ctx.indentTo=state.ctx.start+1);var typeStylePair=state.tokenize(stream,state),type=typeStylePair[0];typeStylePair=typeStylePair[1];var current=stream.current();"space"!==type&&("("===state.lastToken&&null===state.ctx.indentTo?"symbol"===type&&is(current,hasBodyParameter)?state.ctx.indentTo=state.ctx.start+stream.indentUnit:
state.ctx.indentTo="next":"next"===state.ctx.indentTo&&(state.ctx.indentTo=stream.column()),state.lastToken=current);"open"===type?state.ctx={prev:state.ctx,start:stream.column(),indentTo:null}:"close"===type&&(state.ctx=state.ctx.prev||state.ctx);return typeStylePair},indent:function(state){var i=state.ctx.indentTo;return"number"===typeof i?i:state.ctx.start+1},languageData:{closeBrackets:{brackets:["(","[","{",'"']},commentTokens:{line:";;"},autocomplete:[].concat(global,require,module)}};exports.clojure=
global}
//# sourceMappingURL=module$node_modules$$codemirror$legacy_modes$mode$clojure.js.map
