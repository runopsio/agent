...
level:handlerData.level})};Breadcrumbs.prototype._domBreadcrumb=function(handlerData){var keyAttrs="object"===typeof this._options.dom?this._options.dom.serializeAttribute:void 0;"string"===typeof keyAttrs&&(keyAttrs=[keyAttrs]);try{var target=handlerData.event.target?utils_1.htmlTreeAsString(handlerData.event.target,keyAttrs):utils_1.htmlTreeAsString(handlerData.event,keyAttrs)}catch(e){target="\x3cunknown\x3e"}0!==target.length&&core_1.getCurrentHub().addBreadcrumb({category:"ui."+handlerData.name,
message:target},{event:handlerData.event,name:handlerData.name,global:handlerData.global})};Breadcrumbs.prototype._xhrBreadcrumb=function(handlerData){if(handlerData.endTimestamp&&!handlerData.xhr.__sentry_own_request__){var _a=handlerData.xhr.__sentry_xhr__||{},method=_a.method,url=_a.url,status_code=_a.status_code;_a=_a.body;core_1.getCurrentHub().addBreadcrumb({category:"xhr",data:{method,url,status_code},type:"http"},{xhr:handlerData.xhr,input:_a})}};Breadcrumbs.prototype._fetchBreadcrumb=function(handlerData){handlerData.endTimestamp&&
(handlerData.fetchData.url.match(/sentry_key/)&&"POST"===handlerData.fetchData.method||(handlerData.error?core_1.getCurrentHub().addBreadcrumb({category:"fetch",data:handlerData.fetchData,level:types_1.Severity.Error,type:"http"},{data:handlerData.error,input:handlerData.args}):core_1.getCurrentHub().addBreadcrumb({category:"fetch",data:tslib_1.__assign(tslib_1.__assign({},handlerData.fetchData),{status_code:handlerData.response.status}),type:"http"},{input:handlerData.args,response:handlerData.response})))};
Breadcrumbs.prototype._historyBreadcrumb=function(handlerData){var global=utils_1.getGlobalObject(),from=handlerData.from;handlerData=handlerData.to;global=utils_1.parseUrl(global.location.href);var parsedFrom=utils_1.parseUrl(from),parsedTo=utils_1.parseUrl(handlerData);parsedFrom.path||(parsedFrom=global);global.protocol===parsedTo.protocol&&global.host===parsedTo.host&&(handlerData=parsedTo.relative);global.protocol===parsedFrom.protocol&&global.host===parsedFrom.host&&(from=parsedFrom.relative);
core_1.getCurrentHub().addBreadcrumb({category:"navigation",data:{from,to:handlerData}})};Breadcrumbs.id="Breadcrumbs";return Breadcrumbs}();exports.Breadcrumbs=global$jscomp$0}
