...
last.prevSibling):pos=last}return node}function resolveNode(node,pos,side,overlays){for(var _a;node.from==node.to||(1>side?node.from>=pos:node.from>pos)||(-1<side?node.to<=pos:node.to<pos);){let parent=!overlays&&node instanceof TreeNode&&0>node.index?null:node.parent;if(!parent)return node;node=parent}if(overlays)for(let scan=node,parent=scan.parent;parent;scan=parent,parent=scan.parent)scan instanceof TreeNode&&0>scan.index&&(null===(_a=parent.enter(pos,side,!0))||void 0===_a?void 0:_a.from)!=scan.from&&
(node=parent);for(;;){_a=node.enter(pos,side,overlays);if(!_a)return node;node=_a}}function getChildren(node,type,before,after){node=node.cursor;let result=[];if(!node.firstChild())return result;if(null!=before)for(;!node.type.is(before);)if(!node.nextSibling())return result;for(;;){if(null!=after&&node.type.is(after))return result;node.type.is(type)&&result.push(node.node);if(!node.nextSibling())return null==after?result:[]}}function hasChild(tree){return tree.children.some(ch=>ch instanceof TreeBuffer||
!ch.type.isAnonymous||hasChild(ch))}function buildTree(data){function takeNode(parentStart,minPos,children,positions,inRepeat){let {id,start,end,size}=cursor;for(var lookAheadAtStart=lookAhead;0>size;){cursor.next();if(-1==size){children.push(reused[id]);positions.push(start-parentStart);return}if(-3==size){contextHash=id;return}if(-4==size){lookAhead=id;return}throw new RangeError(`Unrecognized record size: ${size}`);}var type=types[id],startPos$jscomp$0=start-parentStart,JSCompiler_temp;if(JSCompiler_temp=
end-start<=maxBufferLength){var buffer=cursor.pos-minPos;minPos=cursor.fork();var start$jscomp$0=JSCompiler_temp=0;let skip=0,minStart=minPos.end-maxBufferLength,result={size:0,start:0,skip:0};a:for(let minPos$jscomp$0=minPos.pos-buffer;minPos.pos>minPos$jscomp$0;){let nodeSize=minPos.size;if(minPos.id==inRepeat&&0<=nodeSize){result.size=JSCompiler_temp;result.start=start$jscomp$0;result.skip=skip;skip+=4;JSCompiler_temp+=4;minPos.next();continue}let startPos=minPos.pos-nodeSize;if(0>nodeSize||startPos<
minPos$jscomp$0||minPos.start<minStart)break;let localSkipped=minPos.id>=minRepeatType?4:0,nodeStart=minPos.start;for(minPos.next();minPos.pos>startPos;){if(0>minPos.size)if(-3==minPos.size)localSkipped+=4;else break a;else minPos.id>=minRepeatType&&(localSkipped+=4);minPos.next()}start$jscomp$0=nodeStart;JSCompiler_temp+=nodeSize;skip+=localSkipped}if(0>inRepeat||JSCompiler_temp==buffer)result.size=JSCompiler_temp,result.start=start$jscomp$0,result.skip=skip;JSCompiler_temp=buffer=4<result.size?
result:void 0}if(JSCompiler_temp){startPos$jscomp$0=new Uint16Array(buffer.size-buffer.skip);type=cursor.pos-buffer.size;for(lookAheadAtStart=startPos$jscomp$0.length;cursor.pos>type;)lookAheadAtStart=copyToBuffer(buffer.start,startPos$jscomp$0,lookAheadAtStart);type=new TreeBuffer(startPos$jscomp$0,end-buffer.start,nodeSet);startPos$jscomp$0=buffer.start-parentStart}else{buffer=cursor.pos-size;cursor.next();parentStart=[];inRepeat=[];minPos=id>=minRepeatType?id:-1;JSCompiler_temp=0;for(start$jscomp$0=
end;cursor.pos>buffer;)0<=minPos&&cursor.id==minPos&&0<=cursor.size?(cursor.end<=start$jscomp$0-maxBufferLength&&(makeRepeatLeaf(parentStart,inRepeat,start,JSCompiler_temp,cursor.end,start$jscomp$0,minPos,lookAheadAtStart),JSCompiler_temp=parentStart.length,start$jscomp$0=cursor.end),cursor.next()):takeNode(start,buffer,parentStart,inRepeat,minPos);0<=minPos&&0<JSCompiler_temp&&JSCompiler_temp<parentStart.length&&makeRepeatLeaf(parentStart,inRepeat,start,JSCompiler_temp,start,start$jscomp$0,minPos,
lookAheadAtStart);parentStart.reverse();inRepeat.reverse();-1<minPos&&0<JSCompiler_temp?(lookAheadAtStart=makeBalanced(type),type=balanceRange(type,parentStart,inRepeat,0,parentStart.length,0,end-start,lookAheadAtStart,lookAheadAtStart)):type=makeTree(type,parentStart,inRepeat,end-start,lookAheadAtStart-end)}children.push(type);positions.push(startPos$jscomp$0)}function makeBalanced(type){return(children,positions,length)=>{let lookAhead=0,lastI=children.length-1,last,lookAheadProp;if(0<=lastI&&(last=
children[lastI])instanceof Tree){if(!lastI&&last.type==type&&last.length==length)return last;if(lookAheadProp=last.prop(NodeProp.lookAhead))lookAhead=positions[lastI]+last.length+lookAheadProp}return makeTree(type,children,positions,length,lookAhead)}}function makeRepeatLeaf(children,positions,base,i,from,to,type,lookAhead){let localChildren=[],localPositions=[];for(;children.length>i;)localChildren.push(children.pop()),localPositions.push(positions.pop()+base-from);children.push(makeTree(nodeSet.types[type],
...
to);for(let i=0,pos=from;;i++){let end=(from=i==endPos.length)?to:endPos[i].from;end>pos&&result.push(new TreeFragment(pos,end,mount.tree,-mounts,frag.from>=pos,frag.to<=end));if(from)break;pos=endPos[i].to}}else result.push(new TreeFragment(from,to,mount.tree,-mounts,frag.from>=mounts,frag.to<=endPos))}return result}Object.defineProperty(exports,"__esModule",{value:!0});let nextPropID=0;class Range{constructor(from,to){this.from=from;this.to=to}}class NodeProp{constructor(config={}){this.id=nextPropID++;
this.perNode=!!config.perNode;this.deserialize=config.deserialize||(()=>{throw Error("This node type doesn't define a deserialize function");})}add(match){if(this.perNode)throw new RangeError("Can't add per-node props to node types");"function"!=typeof match&&(match=NodeType.match(match));return type=>{type=match(type);return void 0===type?null:[this,type]}}}NodeProp.closedBy=new NodeProp({deserialize:str=>str.split(" ")});NodeProp.openedBy=new NodeProp({deserialize:str=>str.split(" ")});NodeProp.group=
new NodeProp({deserialize:str=>str.split(" ")});NodeProp.contextHash=new NodeProp({perNode:!0});NodeProp.lookAhead=new NodeProp({perNode:!0});NodeProp.mounted=new NodeProp({perNode:!0});class MountedTree{constructor(tree,overlay,parser){this.tree=tree;this.overlay=overlay;this.parser=parser}}const noProps=Object.create(null);class NodeType{constructor(name,props,id,flags=0){this.name=name;this.props=props;this.id=id;this.flags=flags}static define(spec){let props=spec.props&&spec.props.length?Object.create(null):
noProps,type=new NodeType(spec.name||"",props,spec.id,(spec.top?1:0)|(spec.skipped?2:0)|(spec.error?4:0)|(null==spec.name?8:0));if(spec.props)for(let src of spec.props)if(Array.isArray(src)||(src=src(type)),src){if(src[0].perNode)throw new RangeError("Can't store a per-node prop on a node type");props[src[0].id]=src[1]}return type}prop(prop){return this.props[prop.id]}get isTop(){return 0<(this.flags&1)}get isSkipped(){return 0<(this.flags&2)}get isError(){return 0<(this.flags&4)}get isAnonymous(){return 0<
(this.flags&8)}is(name){if("string"==typeof name){if(this.name==name)return!0;let group=this.prop(NodeProp.group);return group?-1<group.indexOf(name):!1}return this.id==name}static match(map){let direct=Object.create(null);for(let prop in map)for(let name of prop.split(" "))direct[name]=map[prop];return node=>{for(let groups=node.prop(NodeProp.group),i=-1;i<(groups?groups.length:0);i++){let found=direct[0>i?node.name:groups[i]];if(found)return found}}}}NodeType.none=new NodeType("",Object.create(null),
0,8);class NodeSet{constructor(types){this.types=types;for(let i=0;i<types.length;i++)if(types[i].id!=i)throw new RangeError("Node type ids should correspond to array positions when creating a node set");}extend(...props){let newTypes=[];for(let type of this.types){let newProps=null;for(let source of props){let add=source(type);add&&(newProps||(newProps=Object.assign({},type.props)),newProps[add[0].id]=add[1])}newTypes.push(newProps?new NodeType(type.name,newProps,type.id,type.flags):type)}return new NodeSet(newTypes)}}
const CachedNode=new WeakMap,CachedInnerNode=new WeakMap;class Tree{constructor(type,children,positions,length,props){this.type=type;this.children=children;this.positions=positions;this.length=length;this.props=null;if(props&&props.length){this.props=Object.create(null);for(let [prop,value]of props)this.props["number"==typeof prop?prop:prop.id]=value}}toString(){var mounted=this.prop(NodeProp.mounted);if(mounted&&!mounted.overlay)return mounted.tree.toString();mounted="";for(let ch of this.children){let str=
ch.toString();str&&(mounted&&(mounted+=","),mounted+=str)}return this.type.name?(/\W/.test(this.type.name)&&!this.type.isError?JSON.stringify(this.type.name):this.type.name)+(mounted.length?"("+mounted+")":""):mounted}cursor(pos,side=0){var scope=null!=pos&&CachedNode.get(this)||this.topNode;scope=new TreeCursor(scope);null!=pos&&(scope.moveTo(pos,side),CachedNode.set(this,scope._tree));return scope}fullCursor(){return new TreeCursor(this.topNode,1)}get topNode(){return new TreeNode(this,0,0,null)}resolve(pos,
side=0){pos=resolveNode(CachedNode.get(this)||this.topNode,pos,side,!1);CachedNode.set(this,pos);return pos}resolveInner(pos,side=0){pos=resolveNode(CachedInnerNode.get(this)||this.topNode,pos,side,!0);CachedInnerNode.set(this,pos);return pos}iterate(spec){let {enter,leave,from=0,to=this.length}=spec;for(let c=this.cursor(),get=()=>c.node;;){spec=!1;if(c.from<=to&&c.to>=from&&(c.type.isAnonymous||!1!==enter(c.type,c.from,c.to,get))){if(c.firstChild())continue;c.type.isAnonymous||(spec=!0)}for(;;){spec&&
leave&&leave(c.type,c.from,c.to,get);spec=c.type.isAnonymous;if(c.nextSibling())break;if(!c.parent())return;spec=!0}}}prop(prop){return prop.perNode?this.props?this.props[prop.id]:void 0:this.type.prop(prop)}get propValues(){let result=[];if(this.props)for(let id in this.props)result.push([+id,this.props[id]]);return result}balance(config={}){return 8>=this.children.length?this:balanceRange(NodeType.none,this.children,this.positions,0,this.children.length,0,this.length,(children,positions,length)=>
new Tree(this.type,children,positions,length,this.propValues),config.makeTree||((children,positions,length)=>new Tree(NodeType.none,children,positions,length)))}static build(data){return buildTree(data)}}Tree.empty=new Tree(NodeType.none,[],[],0);class FlatBufferCursor{constructor(buffer,index){this.buffer=buffer;this.index=index}get id(){return this.buffer[this.index-4]}get start(){return this.buffer[this.index-3]}get end(){return this.buffer[this.index-2]}get size(){return this.buffer[this.index-
1]}get pos(){return this.index}next(){this.index-=4}fork(){return new FlatBufferCursor(this.buffer,this.index)}}class TreeBuffer{constructor(buffer,length,set){this.buffer=buffer;this.length=length;this.set=set}get type(){return NodeType.none}toString(){let result=[];for(let index=0;index<this.buffer.length;)result.push(this.childString(index)),index=this.buffer[index+3];return result.join(",")}childString(index){let endIndex=this.buffer[index+3];var type=this.set.types[this.buffer[index]];let result=
type.name;/\W/.test(result)&&!type.isError&&(result=JSON.stringify(result));index+=4;if(endIndex==index)return result;for(type=[];index<endIndex;)type.push(this.childString(index)),index=this.buffer[index+3];return result+"("+type.join(",")+")"}findChild(startIndex,endIndex,dir,pos,side){let {buffer}=this,pick=-1;for(;startIndex!=endIndex&&!(checkSide(side,pos,buffer[startIndex+1],buffer[startIndex+2])&&(pick=startIndex,0<dir));startIndex=buffer[startIndex+3]);return pick}slice(startI,endI,from,to){let b=
this.buffer,copy=new Uint16Array(endI-startI);for(let i=startI,j=0;i<endI;)copy[j++]=b[i++],copy[j++]=b[i++]-from,copy[j++]=b[i++]-from,copy[j++]=b[i++]-startI;return new TreeBuffer(copy,to-from,this.set)}}class TreeNode{constructor(node,_from,index,_parent){this.node=node;this._from=_from;this.index=index;this._parent=_parent}get type(){return this.node.type}get name(){return this.node.type.name}get from(){return this._from}get to(){return this._from+this.node.length}nextChild(i,dir,pos,side,mode=
0){for(let parent=this;;){for(let {children,positions}=parent.node,e=0<dir?children.length:-1;i!=e;i+=dir){let next=children[i],start=positions[i]+parent._from;if(checkSide(side,pos,start,start+next.length))if(next instanceof TreeBuffer){if(mode&2)continue;let index=next.findChild(0,next.buffer.length,dir,pos-start,side);if(-1<index)return new BufferNode(new BufferContext(parent,next,i,start),null,index)}else if(mode&1||!next.type.isAnonymous||hasChild(next)){let mounted;if(!(mode&1)&&next.props&&
(mounted=next.prop(NodeProp.mounted))&&!mounted.overlay)return new TreeNode(mounted.tree,start,i,parent);i=new TreeNode(next,start,i,parent);return mode&1||!i.type.isAnonymous?i:i.nextChild(0>dir?next.children.length-1:0,dir,pos,side)}}if(mode&1||!parent.type.isAnonymous)return null;i=0<=parent.index?parent.index+dir:0>dir?-1:parent._parent.node.children.length;parent=parent._parent;if(!parent)return null}}get firstChild(){return this.nextChild(0,1,0,4)}get lastChild(){return this.nextChild(this.node.children.length-
1,-1,0,4)}childAfter(pos){return this.nextChild(0,1,pos,2)}childBefore(pos){return this.nextChild(this.node.children.length-1,-1,pos,-2)}enter(pos,side,overlays=!0,buffers=!0){let mounted;if(overlays&&(mounted=this.node.prop(NodeProp.mounted))&&mounted.overlay){overlays=pos-this.from;for(let {from,to}of mounted.overlay)if((0<side?from<=overlays:from<overlays)&&(0>side?to>=overlays:to>overlays))return new TreeNode(mounted.tree,mounted.overlay[0].from+this.from,-1,this)}return this.nextChild(0,1,pos,
side,buffers?0:2)}nextSignificantParent(){let val=this;for(;val.type.isAnonymous&&val._parent;)val=val._parent;return val}get parent(){return this._parent?this._parent.nextSignificantParent():null}get nextSibling(){return this._parent&&0<=this.index?this._parent.nextChild(this.index+1,1,0,4):null}get prevSibling(){return this._parent&&0<=this.index?this._parent.nextChild(this.index-1,-1,0,4):null}get cursor(){return new TreeCursor(this)}get tree(){return this.node}toTree(){return this.node}resolve(pos,
side=0){return resolveNode(this,pos,side,!1)}resolveInner(pos,side=0){return resolveNode(this,pos,side,!0)}enterUnfinishedNodesBefore(pos){return enterUnfinishedNodesBefore(this,pos)}getChild(type,before=null,after=null){type=getChildren(this,type,before,after);return type.length?type[0]:null}getChildren(type,before=null,after=null){return getChildren(this,type,before,after)}toString(){return this.node.toString()}}class BufferContext{constructor(parent,buffer,index,start){this.parent=parent;this.buffer=
...
pos&&(pos=Math.min(pos,this.inner[i].parse.parsedPos));return pos}stopAt(pos){this.stoppedAt=pos;if(this.baseParse)this.baseParse.stopAt(pos);else for(let i=this.innerDone;i<this.inner.length;i++)this.inner[i].parse.stopAt(pos)}startInner(){let fragmentCursor=new FragmentCursor(this.fragments),overlay=null,covered=null,cursor=new TreeCursor(new TreeNode(this.baseTree,this.ranges[0].from,0,null),1);a:for(let nest,isCovered;null==this.stoppedAt||cursor.from<this.stoppedAt;){var enter=!0;let range;if(fragmentCursor.hasNode(cursor)){if(overlay&&
(enter=overlay.mounts.find(m=>m.frag.from<=cursor.from&&m.frag.to>=cursor.to&&m.mount.overlay)))for(let r of enter.mount.overlay){var from=r.from+enter.pos,to=r.to+enter.pos;from>=cursor.from&&to<=cursor.to&&overlay.ranges.push({from,to})}enter=!1}else covered&&(isCovered=checkCover(covered.ranges,cursor.from,cursor.to))?enter=2!=isCovered:!cursor.type.isAnonymous&&cursor.from<cursor.to&&(nest=this.nest(cursor,this.input))?(cursor.tree||materialize(cursor),from=fragmentCursor.findMounts(cursor.from,
nest.parser),"function"==typeof nest.overlay?overlay=new ActiveOverlay(nest.parser,nest.overlay,from,this.inner.length,cursor.from,cursor.tree,overlay):(to=punchRanges(this.ranges,nest.overlay||[new Range(cursor.from,cursor.to)]),to.length&&this.inner.push(new InnerParse(nest.parser,nest.parser.startParse(this.input,enterFragments(from,to),to),nest.overlay?nest.overlay.map(r=>new Range(r.from-cursor.from,r.to-cursor.from)):null,cursor.tree,to)),nest.overlay?to.length&&(covered={ranges:to,depth:0,
prev:covered}):enter=!1)):overlay&&(range=overlay.predicate(cursor))&&(!0===range&&(range=new Range(cursor.from,cursor.to)),range.from<range.to&&overlay.ranges.push(range));if(enter&&cursor.firstChild())overlay&&overlay.depth++,covered&&covered.depth++;else for(;!cursor.nextSibling();){if(!cursor.parent())break a;overlay&&!--overlay.depth&&(enter=punchRanges(this.ranges,overlay.ranges),enter.length&&this.inner.splice(overlay.index,0,new InnerParse(overlay.parser,overlay.parser.startParse(this.input,
enterFragments(overlay.mounts,enter),enter),overlay.ranges.map(r=>new Range(r.from-overlay.start,r.to-overlay.start)),overlay.target,enter)),overlay=overlay.prev);covered&&!--covered.depth&&(covered=covered.prev)}}}}class StructureCursor{constructor(root,offset){this.offset=offset;this.done=!1;this.cursor=root.fullCursor()}moveTo(pos){let {cursor}=this,p=pos-this.offset;for(;!this.done&&cursor.from<p;)cursor.to>=pos&&cursor.enter(p,1,!1,!1)||cursor.next(!1)||(this.done=!0)}hasNode(cursor){this.moveTo(cursor.from);
if(!this.done&&this.cursor.from+this.offset==cursor.from&&this.cursor.tree)for(let tree=this.cursor.tree;;){if(tree==cursor.tree)return!0;if(tree.children.length&&0==tree.positions[0]&&tree.children[0]instanceof Tree)tree=tree.children[0];else break}return!1}}class FragmentCursor{constructor(fragments){var _a;this.fragments=fragments;this.fragI=this.curTo=0;fragments.length?(fragments=this.curFrag=fragments[0],this.curTo=null!==(_a=fragments.tree.prop(stoppedInner))&&void 0!==_a?_a:fragments.to,this.inner=
new StructureCursor(fragments.tree,-fragments.offset)):this.curFrag=this.inner=null}hasNode(node){for(;this.curFrag&&node.from>=this.curTo;)this.nextFrag();return this.curFrag&&this.curFrag.from<=node.from&&this.curTo>=node.to&&this.inner.hasNode(node)}nextFrag(){var _a;this.fragI++;if(this.fragI==this.fragments.length)this.curFrag=this.inner=null;else{let frag=this.curFrag=this.fragments[this.fragI];this.curTo=null!==(_a=frag.tree.prop(stoppedInner))&&void 0!==_a?_a:frag.to;this.inner=new StructureCursor(frag.tree,
-frag.offset)}}findMounts(pos,parser){var _a;let result=[];if(this.inner)for(this.inner.cursor.moveTo(pos,1),pos=this.inner.cursor.node;pos;pos=pos.parent){let mount=null===(_a=pos.tree)||void 0===_a?void 0:_a.prop(NodeProp.mounted);if(mount&&mount.parser==parser)for(let i=this.fragI;i<this.fragments.length;i++){let frag=this.fragments[i];if(frag.from>=pos.to)break;frag.tree==this.curFrag.tree&&result.push({frag,pos:pos.from-frag.offset,mount})}}return result}}exports.DefaultBufferLength=1024;exports.MountedTree=
MountedTree;exports.NodeProp=NodeProp;exports.NodeSet=NodeSet;exports.NodeType=NodeType;exports.Parser=Parser;exports.Tree=Tree;exports.TreeBuffer=TreeBuffer;exports.TreeCursor=TreeCursor;exports.TreeFragment=TreeFragment;exports.parseMixed=function(nest){return(parse,input,fragments,ranges)=>new MixedParse(parse,nest,input,fragments,ranges)}}
